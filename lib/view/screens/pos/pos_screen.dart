    ...
    return Scaffold(
      ...
      body: GetBuilder<PosController>(
        builder: (posController) {
          ...
          if (_restaurant != null) {
            posController.cartList.forEach((cartModel) {
              ...
              double _dis = (_restaurant.discount != null &&
                      DateConverter.isAvailable(_restaurant.discount.startTime,
                          _restaurant.discount.endTime,
                          isoTime: true))
                  ? _restaurant.discount.discount
            ...
            if (_restaurant.discount != null) {
              if (_restaurant.discount.maxDiscount != 0 &&
                  _restaurant.discount.maxDiscount < _discount) {
                _discount = _restaurant.discount.maxDiscount;
              }
              if (_restaurant.discount.minPurchase != 0 &&
                  _restaurant.discount.minPurchase > (_itemPrice + _addOns)) {
                _discount = 0;
              }
            }
            _orderAmount = (_itemPrice - _discount) + _addOns;
            _tax = PriceConverter.calculation(
                _orderAmount, _restaurant.tax, 'percent', 1);
          }
          ...
          return posController.cartList.length > 0
              ? Column(
                  children: [
                    Expanded(
                      child: Scrollbar(
